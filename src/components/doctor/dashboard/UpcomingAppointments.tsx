    ...
    <Card className="h-full">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>{t("stats.upcomingAppointmentsDesc")}</CardDescription>
        </div>
      ...
      <CardContent>
        {appointments.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground border rounded-lg border-dashed">
            ...
            <p>{t("emptyStates.noUpcomingAppointments")}</p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col text-xs text-muted-foreground mt-0.5">
                      <span>{apt.schedule?.startDateTime ? format(new Date(apt.schedule.startDateTime), "MMM dd, yyyy") : "TBD"}</span>
                      <span>{apt.schedule?.startDateTime ? format(new Date(apt.schedule.startDateTime), "hh:mm a") : ""}</span>
                    </div>
                  </div>
                </div>
